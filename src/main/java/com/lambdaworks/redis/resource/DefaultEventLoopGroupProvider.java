...
public class DefaultEventLoopGroupProvider implements EventLoopGroupProvider {
    ...
    public static <T extends EventExecutorGroup> EventExecutorGroup createEventLoopGroup(Class<T> type, int numberOfThreads) {
        if (DefaultEventExecutorGroup.class.equals(type)) {
        	Executor excutor = Executors.newCachedThreadPool(new DefaultThreadFactory("lettuce-eventExecutorLoop", true));
            return new DefaultEventExecutorGroup(numberOfThreads, excutor);
        }

        if (NioEventLoopGroup.class.equals(type)) {
        	Executor excutor = Executors.newCachedThreadPool(new DefaultThreadFactory("lettuce-nioEventLoop", true));
        	return new NioEventLoopGroup(numberOfThreads, excutor);
        }
